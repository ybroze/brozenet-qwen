...
Ollama on Cloud Run with an L4 GPU. Deploy with `./deploy.sh broze-net`.

## Architecture

- `app/Dockerfile` — Ollama + nginx + envsubst
- `app/entrypoint.sh` — renders nginx config, starts nginx and Ollama, pulls model
- `app/nginx.conf.template` — bearer token auth, reverse proxy to Ollama on localhost:11434
- `ansible/playbook.yml` — provisions everything: APIs, SA, Secret Manager, Artifact Registry, Cloud Build, Cloud Run, scheduler, domain
...
- Secrets (project_id) are CLI arguments, not files.
- Domain: qwen.broze.net, DNS via Cloudflare.
